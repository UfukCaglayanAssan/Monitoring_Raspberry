...
if grep -q "ftp_backup.py" /tmp/current_cron; then
    ...
    # Her gün saat 11:50'de çalışacak cron job ekle (Türkiye saati)
    echo "50 11 * * * /usr/bin/python3 $SCRIPT_DIR/ftp_backup.py >> $SCRIPT_DIR/ftp_backup.log 2>&1" >> /tmp/current_cron
    
    # Yeni crontab'ı yükle
    crontab /tmp/current_cron
    
    echo "✅ FTP backup cron job'u eklendi!"
    echo "⏰ Her gün saat 11:50'de çalışacak (Türkiye saati)"
